...
export default async function VenueRankingsPage({
  params,
}: {
  params: Promise<{
    country: string;
  ...
  if (!session?.user?.email || !session?.user?.id) {
    redirect(
      `/login?callbackUrl=/${country}/${lang}/${city}/venues/${venueId}/competitions/${competitionId}`
    );
  }

  const [venue, venueMembers, competition] = await Promise.all([
    alwaysfullybooked.venues.publicFind({ venueId }),
    api.venueMembers.search({ venueId }),
  ...
  const playerMap = new Map(
    venueMembers.map((vm) => [vm.playerId, vm.playerName])
  );
  ...
  const pendingGames = games.filter(
    (game) =>
      game.status === "pending" &&
      [
        ...game.winnerTeam.map((player) => player.id),
        ...game.playerTeam.map((player) => player.id),
      ].includes(session.user.id)
  );
  const approvedGames = games.filter((game) => game.status === "approved");

  const playedRankings = leaderboard.filter(
  ...
  return (
    <div className="px-2 py-4 sm:px-6 lg:px-8">
      <div className="text-center space-y-2 mb-6 sm:mb-12">
        ...
        <h2 className="text-xl font-bold tracking-tight sm:text-2xl md:text-3xl capitalize">
          {competition?.name}
        </h2>
      </div>
      ...
      {isMember && competition?.matchType && (
        <div className="text-center space-y-2 mb-6 sm:mb-12">
          <h2 className="text-lg font-bold tracking-tight sm:text-xl md:text-2xl capitalize">
            Just Played?
          </h2>
          <AddVenueGame
            competitionId={competitionId}
          ...
          <p className="text-sm text-muted-foreground">
            Winner adds final score. Player approves.
          </p>
        </div>
      )}
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-xs sm:text-sm text-muted-foreground mt-3">
                          {game.playedDate}
                        </span>
